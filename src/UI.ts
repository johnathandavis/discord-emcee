import {
    StringSelectMenuOptionBuilder, StringSelectMenuBuilder, ActionRowBuilder, ButtonBuilder,
    ButtonStyle, BaseMessageOptions
} from "discord.js";
import { EmceeUserInterface, StateInput, StateValue, OptionStateInput, BooleanStateInput, IOption } from "./Shared";
...
export function createUI<T extends MCSchema<MCRawShape>>(
    ...
    validator: (s: Infer<T>) => boolean): BaseMessageOptions {
...
export const createSubmit = (config: SubmitConfig, enabled: boolean): Component => {
    if (!config) {
        config = {};
    }
...
export const createUIInput = (input: StateInput, currentValue: StateValue): Component => {
...
type ButtonStyleConfig = Required<Exclude<BooleanStateInput['trueStyle'], undefined>>;
const getStyle = (i: BooleanStateInput, currentValue: boolean): ButtonStyleConfig => {
...
const createButton = (input: BooleanStateInput, currentValue: boolean): ButtonBuilder => {
...
const createOption = (input: OptionStateInput, currentValue: IOption<any>): StringSelectMenuBuilder => {
    const selectedValue = currentValue ?? input.value;
    const eb = new StringSelectMenuBuilder()
