...
import { StateInput, StateValue, StateDefinition,
Updater, InputUpdateArgs, InputUpdatedHandler, ValidationStateChangedArgs } from "./Shared";
import evt from 'events';

type InternalCollector = InteractionCollector<ButtonInteraction | StringSelectMenuInteraction>;
...
function toComponentType<T extends StateInput>(t: StateInput['type']): ComponentTypeOf<T> {
    switch (t) {
        case 'Boolean':
...
function createAttachment<T extends StateInput>(k: T, r: InteractionResponse, handler: InputUpdatedHandler<T>): InternalCollector {
...
class State {
    ...
    monitorStateUpdates = (r: InteractionResponse) => {
        ...
        this._definition.inputs.forEach(si => {
            ...
            const collector = createAttachment(internalItem.item, r, this.onStateItemUpdated);
    ...
    [Symbol.dispose] = () => {
        ...
        if (!this._response) {
            console.debug('Skipping state disposal since no InteractionResponse set...');
            return;
        }
